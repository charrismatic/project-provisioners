...
add-apt-repository ppa:ondrej/php -y     # >> $VAGRANT_BUILD_LOG 2>&1      ## PHP 7
...
# TOOLS
apt-get install htop nano unzip
# LIBS
...
apt-get -y install mysql-server mysql-server-5.7     # >> $VAGRANT_BUILD_LOG 2>&1
...
mysql -uroot -p "SECRET" -e "create user '$ACTIVE_SITE'@'10.0.2.2' identified by '$ACTIVE_SITE'; grant all privileges on *.* to '$ACTIVE_SITE'@'10.0.2.2' with grant option; flush privileges;"
